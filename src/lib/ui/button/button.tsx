...
export const StyledButton = styled.button<{
  ...
  &:enabled {
    background-color: ${({ theme, $variant, $tone }) => {
      const themeColor = (theme.components.button.base as any)[$variant]?.[
        $tone
      ]?.bg?.color?.default;
  ...
  &:disabled {
    ...
    background-color: ${({ theme, $variant, $tone, $loading }) =>
      $loading
        ? (theme.components.button.base as any)[$variant]?.[$tone]?.bg?.color
            ?.default || "transparent"
        : (theme.components.button.base as any)[$variant]?.[$tone]?.bg?.color
    ...
    border-color: ${({ theme, $variant, $tone, $loading }) =>
      $loading
        ? (theme.components.button.base as any)[$variant]?.[$tone]?.border
...
const ButtonContent = styled.span<{ $loading: boolean }>`
  ...
  opacity: ${({ theme, $loading }) =>
    $loading ? theme.semantic.opacity.hidden : theme.semantic.opacity.visible};
...
const ButtonLabel = styled.span<{
  ...
  gap: 0px;
  padding-inline: ${({ theme, $size }) =>
    theme.components.button[$size].label.paddingX};
  padding-block: 0px;
